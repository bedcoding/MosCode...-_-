...
BinTree* copyBinTree(BinTree* pSource)
{
	...
	if (pSource != NULL) {
		...
		if (pReturn != NULL) {
			...
			printf("오류, 메모리 할당(Error, memory allocation), copyBinTree()\n");
		}
	}
	...
	if (pSourceNode != NULL) {
		...
		if (pReturn != NULL) {
			*pReturn = *pSourceNode;
			...
			printf("오류, 메모리 할당(Error, memory allocation), copyBinTreeNode()\n");
		}
	}
...
void displayBinTree(BinTree *pTree)
{
	if (pTree != NULL) {
		...
		printf("트리가 NULL입니다(Tree : null)\n");
	}
}


// 모스 부호를 출력하기 위해 이곳을 수정
void displayBinTreeNode(BinTreeNode *pNode, int level, char type)
{
	...
	if (pNode != NULL) {
		printf("-[%i,%c] %c      \t( MOS: %s ) \n", level, type, pNode->data, pNode->morse_code);  // 여기서 %s 한개 추가해주니 모스 부호가 출력됐다!
