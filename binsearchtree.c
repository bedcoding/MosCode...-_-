...
char insertElementBST(BinSearchTree* pBinSearchTree, BinSearchTreeNode element)
{
	...
	while(pParentNode != NULL) {
		if (element.key == pParentNode->key) {
			printf("오류,중복된 키-[%c],insertElementBST()\n", element.key);
			ret = FALSE;
			return ret;
		}
...
char deleteElementBST(BinSearchTree* pBinSearchTree, char key)
{
	...
	if (pDelNode == NULL) {
		printf("오류,존재하지 않는 키-[%c],deleteElementBST()\n", key);
		ret = FALSE;
		return ret;
	}
...
BinSearchTreeNode* searchInternalRecursiveBST(BinSearchTreeNode* pTreeNode, char key)
{
	BinSearchTreeNode* pReturn = NULL;
